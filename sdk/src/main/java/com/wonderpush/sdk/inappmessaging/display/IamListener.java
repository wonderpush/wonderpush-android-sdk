 ...
 *
 * <ul>
 *   <li>{@link IamListener#onIamTrigger()} is called before the message is rendered. The method
 *       is called repeatedly for a message if it needs to be re-rendered during activity
 *       transitions
 *   <li>{@link IamListener#onIamClick()} is called when a message with a configured action is
 *       clicked. If the clicked message does not have a configured action, it is dismissed and
 *       {@link IamListener#onIamClick()} is invoked
